...
def create_pivot_tables(df):
    ...
    pivot_tables['Physical File Discrepancy'] = pd.pivot_table(df,
                                                         values='State_Count',
                                                         index=['State', 'Cluster'],
                                                         columns='Slab',
                                                         aggfunc='sum',
                                                         fill_value=0)
    pivot_tables['Physical File Discrepancy'] = pivot_tables['Physical File Discrepancy'].reindex(columns=column_order, fill_value=0)
    pivot_tables['Physical File Discrepancy']['Grand Total'] = pivot_tables['Physical File Discrepancy'].sum(axis=1)
    grand_total_row = pivot_tables['Physical File Discrepancy'].sum(axis=0)
    grand_total_row.name = ('Total', 'Total')
    pivot_tables['Physical File Discrepancy'] = pd.concat([pivot_tables['Physical File Discrepancy'], grand_total_row.to_frame().T])
...
# Streamlit app
def main():
    st.title("PHYSICAL FILE DISCREPANCY")
    ...
    if uploaded_file is not None:
        ...
        headers = {
            'Physical File Discrepancy': 'Physical File Discrepancy',
            'State_Count': 'Physical File Discrepancy',
